...
"refresh",b.currentState);b.options.debug&&window.console.debug("Ajaxy.request: There has been no considerable change",[this,arguments],[b.currentState,h,a]);return true}b.ajaxQueue.push(a);if(b.ajaxQueue.length!==1)return false;b.options.analytics&&b.track(h);b.currentState=h;f=h.controller||b.match(a)||undefined;h.controller=f;h.Request.url=h.Request.url||h.clean.location;b.storeState(h);b.trigger(f,"request");f={data:h.Request.data,url:h.Request.url,type:"post",success:function(k,o,q){b.options.debug&&
window.console.debug("Ajaxy.request.success:",[this,arguments]);if(k.Ajaxy.redirected){var l=b.extractState(k.Ajaxy.redirected.to);b.redirected={status:true,from:a,to:l};d.go(l)}b.ajaxQueue.shift();if((l=b.ajaxQueue.pop())&&l!==a){b.ajaxQueue=[];b.stateChange(l);return false}h.Request.XMLHttpRequest=q;h.Response.data=k;h.Error.data={};l=k.controller||h.controller||null;if(l===null){l="_generic";window.console.warn("Ajaxy.request.success.controller: The controller was unable to be determined, defaulted to _generic.",
[this,arguments],[k.controller,h.controller])}if(h.Response.callback)if(h.Response.callback.apply(h,arguments)||l==="callback")return true;return b.trigger(l,"response",h)},error:function(k,o,q,l,r){b.options.debug&&window.console.debug("Ajaxy.request.error:",[this,arguments]);b.ajaxQueue.shift();var p=b.ajaxQueue.pop();if(p&&p!==a){b.ajaxQueue=[];b.stateChange(p);return false}h.Request.XMLHttpRequest=k;h.Response.data=r;h.Error.data=l;p=r.controller||h.controller||null;if(p===null){p="_generic";
window.console.warn("Ajaxy.request.error.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[r.controller,h.controller])}if(h.Error.callback)if(h.Error.callback.apply(h,arguments)||p==="callback")return true;return b.trigger(p,"error",h)},complete:function(k,o){b.options.debug&&window.console.debug("Ajaxy.request.complete:",[this,arguments]);if(k)h.Request.XMLHttpRequest=k;if(o)h.Request.textStatus=o}};if(h.isForm){var i=c(h.el);if(i.attr("enctype")===
"multipart/form-data"){e="ajaxy_form_iframe_"+Math.floor(Math.random()*99999);var j=c('<iframe style="display:none" src="about:blank" id="'+e+'" name="'+e+'" >').appendTo("body").hide(),m=c('<input type="hidden" name="ajax" value="true"/>').appendTo(i),n=c('<input type="hidden" name="Ajaxy[form]" value="true"/>').appendTo(i);j.bind("load",function(){if((this.document||this.currentDocument||this.contentWindow.document).location.href==="about:blank")return true;var k=j.contents().find(".response").val(),
o=false;try{o=c.parseJSON(k)}catch(q){window.console.error("Ajaxy.request.form: Invalid Response.",[this,arguments],[k])}o?request.success(o):request.error(o);i.removeAttr("target");j.remove();m.remove();n.remove()});i.attr("target",e);i.submit();g=i.values();f.data=c.extend(true,f.data,g||{});e=true}else{g=i.values();f.data=c.extend(true,f.data,g||{})}}g=true;h.Request=f;e||(g=b.ajax(f));return g}},prepareResponseData:function(a,b){var d=c.Ajaxy,e={},f=b.XMLHttpRequest.responseText||false;if(f)f=
String(f).trim();f||(f=false);if(typeof a==="object")e=a;else{if(typeof a==="string")f=a.trim();if(!f)if(f=b.XMLHttpRequest.responseText||"")f=String(f).trim();try{e=c.parseJSON(f)}catch(g){if(!d.options.support_text){window.console.error("Ajaxy.prepareResponseData: Response has been received as text, however support_text is set to false.",[this,arguments]);window.console.trace();return false}e=d.htmlCompat(f);var h=c(e),i=h.find("#ajaxy-head"),j=h.find("#ajaxy-body"),m=h.find("#ajaxy-title");h=h.find("#ajaxy-controller");
m=m.length?m.text():"";i=i.length?i.htmlAndSelf():"";var n=j.length?j.htmlAndSelf():"";j=j.length?j.html():e;e={controller:h.length?h.text().trim():null,html:e,title:m,head:i,body:n,content:j}}}if(!(e.responseText||0))e.responseText=f;e=c.extend(true,{},d.defaults.State.Response.data,e||{});e.Ajaxy=e.Ajaxy||{};e.Ajaxy.redirected=e.Ajaxy.redirected||false;return e},prepareErrorData:function(a,b){if(typeof a!=="object")a={};if(!(a.textStatus||0))a.textStatus=b.textStatus;if(!(a.errorThrown||0))a.errorThrown=
b.errorThrown;if(!(a.error||0))a.error=b.errorThrown||(b.XMLHttpRequest.status||"Error")+": "+(b.XMLHttpRequest.statusText||"Unknown");return a},ajax:function(a){var b=c.Ajaxy;b.options.debug&&window.console.debug("Ajaxy.ajax:",[this,arguments]);var d={};d.success=a.success||function(){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.success:",[this,arguments]);c(".error").empty()};d.error=a.error||function(f,g,h,i){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.error:",[this,
arguments]);c(".error").html(i.error)};d.complete=a.complete||function(){b.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.complete:",[this,arguments])};delete a.success;delete a.error;delete a.complete;var e={type:"post",dataType:b.options.support_text?"text":"json"};c.extend(true,e,a);e.success=function(f,g,h){b.options.debug&&window.console.debug("Ajaxy.ajax.request.success:",[this,arguments]);b.options.debug&&window.console.debug("Ajaxy.ajax.success:",[this,arguments]);f=b.prepareResponseData(f,
{textStatus:g,XMLHttpRequest:h});var i=false;switch(true){case Boolean(typeof f.controller==="undefined"):i="No controller specified";break;case Boolean(typeof f.success!=="undefined"&&!f.success):i="Success value is set false";break;case Boolean(typeof f.error!=="undefined"&&f.error):i="Custom Error: "+String(f.error);break}if(i)return e.error.apply(this,[h,g,undefined,{error:i},f]);return d.success.apply(this,[f,g,h])};e.error=function(f,g,h,i,j){b.options.debug&&window.console.debug("Ajaxy.ajax.request.error:",
[this,arguments]);j=b.prepareResponseData(j,{XMLHttpRequest:f,errorThrown:h,textStatus:g});i=b.prepareErrorData(i,{XMLHttpRequest:f,errorThrown:h,textStatus:g});return d.error.apply(this,[f,g,i.errorThrown,i,j])};return c.ajax(e)},configure:function(a){var b=c.Ajaxy,d=c.History;a=a||{};if(typeof a!=="object")window.console.error("Ajaxy.configure: Invalid Options Passed",[this,arguments]);else{var e;if(typeof a.Controllers==="object"){e=a.Controllers;delete a.Controllers}else if(typeof a.controllers===
...
c(function(){c(".ajaxy").removeAjaxy()})}b.aliases=[];c.each(b.options.aliases,function(g,h){c.each(h,function(i,j){b.aliases[j]=h})});b.addControllers(e);b.onConfigured(true);return true}},construct:function(){var a=c.Ajaxy,b=c.History;if(!a.isConstructed){a.isConstructed=true;b.bind(function(d){return a.stateChange(d)});c.Sparkle&&c.Sparkle.addExtension("ajaxy",function(){c(this).ajaxify()});c.fn.ajaxy=a.fn.ajaxify;c.each(a.fn,function(d,e){c.fn[d]=e});c(function(){a.onDocumentReady(true)});a.onReady(function(){a.options.auto_ajaxify&&
c("body").ajaxify()});a.onConfigured(function(){a.onDocumentReady(function(){a.onReady(true)})});return true}},ajaxifyController:function(a){var b=c.Ajaxy.getController(a);if(b&&(b.selector||0))c(function(){c(b.selector).data("ajaxy-controller",a).addAjaxy()});return true},fn:{ajaxify:function(){var a=c.Ajaxy,b=c(this);c.each(a.Controllers,function(f){a.ajaxifyController(f)});if(a.options.track_all_internal_links){var d=b.findAndSelf("a[href^=/],a[href^=./]");if(a.options.root_url){var e=c("a[href^="+
...
b.ajaxifyHelpers.form);return d},removeAjaxy:function(a){var b=c.Ajaxy,d=c(this);a=c.extend({permanently:true},a);var e=d.findAndSelf("a.ajaxy").removeClass("ajaxy ajaxy-has").unbind("click",b.ajaxifyHelpers.a);b=d.findAndSelf("form.ajaxy").removeClass("ajaxy ajaxy-has").unbind("submit",b.ajaxifyHelpers.form);a.permanently&&e.add(b).addClass("no-ajaxy");return d}},ajaxifyHelpers:{a:function(a){var b=c.Ajaxy,d=c(this),e=b.extractRelativeUrl(d.attr("href")).replace(/^\/?\.\//,"/"),f=b.extractState(e);
e=b.extractAnchor(e);if("/"+e===f||e===f)e="";var g=!d.hasClass(b.options.no_log_class);d=d.data("ajaxy-controller")||null;b.go({state:f,controller:d,log:g,anchor:e,el:this});a.stopPropagation();a.preventDefault();return false},form:function(a){var b=c.Ajaxy,d=c(this),e=d.attr("disabled");if(e=e||e==="false")return false;if(d.attr("target"))return true;d=b.extractRelativeUrl(d.attr("action")).replace(/^\/?\.\//,"/");d=b.extractState(d);b.go({state:d,el:this});a.stopPropagation();a.preventDefault();
return false}},htmlCompat:function(a){return String(a).replace(/<\!DOCTYPE[^>]*>/i,"").replace(/<(html|head|body|title|meta)/gi,'<div id="ajaxy-$1"').replace(/<\/(html|head|body|title|meta)/gi,"</div")},stateChange:function(a){c.Ajaxy.request(a)},onConfigured:function(){return c.promise({object:this,handlers:"onConfiguredHandlers",flag:"isConfigured",arguments:arguments})},onDocumentReady:function(){return c.promise({object:this,handlers:"onDocumentReadyHandlers",flag:"isDocumentReady",arguments:arguments})},
onReady:function(){return c.promise({object:this,handlers:"onReadyHandlers",flag:"isReady",arguments:arguments})}};c.Ajaxy.construct()}})(jQuery);
