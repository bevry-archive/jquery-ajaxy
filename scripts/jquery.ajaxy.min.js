...
true){var f=b.options.root_url+b.options.base_url+"#"+b.options.relative_url;if(d=d.getHash())f+="?anchor="+d;document.location=f}else if(b.options.redirect==="disable")b.bind=function(){};b.bind(e);return true}},construct:function(){var a=c.Ajaxy,b=c.History;if(!a.constructed){a.constructed=true;b.bind(function(d){return a.stateChange(d)});c.Sparkle&&c.Sparkle.addExtension("ajaxy",function(){c(this).ajaxify()});c.fn.ajaxy=a.fn.ajaxify;c.each(a.fn,function(d,e){c.fn[d]=e});c(function(){a.domReady();
b.domReady()});return true}},domReady:function(){c.Ajaxy.options.auto_ajaxify&&c("body").ajaxify();return true},fn:{ajaxify:function(){var a=c.Ajaxy,b=c(this);c.each(a.Controllers,function(f){a.ajaxifyController(f)});if(a.options.track_all_internal_links){var d=b.findAndSelf("a[href^=/],a[href^=./]");if(a.options.root_url){var e=c("a[href^="+a.options.root_url+"]");d=d.add(e);delete e}d=d.filter(":not(.ajaxy,.no-ajaxy)").addClass("ajaxy");delete d}a.options.track_all_anchors&&b.findAndSelf("a[href^=#]:not(.ajaxy,.no-ajaxy)").addClass("ajaxy");
