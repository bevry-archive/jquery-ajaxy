...
d.compatMode=="BackCompat"?d.body:d.documentElement})};b.fn.scrollTo=function(d,e,f){if(typeof e=="object"){f=e;e=0}if(typeof f=="function")f={onAfter:f};if(d=="max")d=9E9;f=b.extend({},c.defaults,f);e=e||f.speed||f.duration;f.queue=f.queue&&f.axis.length>1;if(f.queue)e/=2;f.offset=a(f.offset);f.over=a(f.over);return this._scrollable().each(function(){function g(l){j.animate(k,e,f.easing,l&&function(){l.call(this,d,f)})}var h=this,j=b(h),i=d,n,k={},m=j.is("html,body");switch(typeof i){case "number":case "string":if(/^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(i)){i=
a(i);break}i=b(i,this);case "object":if(i.is||i.style)n=(i=b(i)).offset()}b.each(f.axis.split(""),function(l,r){var o=r=="x"?"Left":"Top",p=o.toLowerCase(),q="scroll"+o,s=h[q],t=c.max(h,r);if(n){k[q]=n[p]+(m?0:s-j.offset()[p]);if(f.margin){k[q]-=parseInt(i.css("margin"+o))||0;k[q]-=parseInt(i.css("border"+o+"Width"))||0}k[q]+=f.offset[p]||0;if(f.over[p])k[q]+=i[r=="x"?"width":"height"]()*f.over[p]}else{o=i[p];k[q]=o.slice&&o.slice(-1)=="%"?parseFloat(o)/100*t:o}if(/^\d+$/.test(k[q]))k[q]=k[q]<=0?
0:Math.min(k[q],t);if(!l&&f.queue){s!=k[q]&&g(f.onAfterFirst);delete k[q]}});g(f.onAfter)}).end()};c.max=function(d,e){var f=e=="x"?"Width":"Height",g="scroll"+f;if(!b(d).is("html,body"))return d[g]-b(d)[f.toLowerCase()]();f="client"+f;var h=d.ownerDocument.documentElement,j=d.ownerDocument.body;return Math.max(h[g],j[g])-Math.min(h[f],j[f])};b.fn.ScrollTo=function(){for(var d=b(this),e=d.parent(),f=false;e.length===1&&!e.is("body");){var g=e.get(0);if(e.css("overflow")!=="visible"&&g.scrollHeight!==
g.clientHeight){f=true;break}e=e.parent()}if(f){e.scrollTo(d);e.ScrollTo()}else b.scrollTo(d);return d}})(jQuery);
(function(b){if(b.History)window.console.warn("$.History has already been defined...");else{b.History={options:{debug:false},state:"",$window:null,$iframe:null,handlers:{generic:[],specific:{}},extractHash:function(a){return a.replace(/^[^#]*#/,"").replace(/^#+|#+$/,"")},extractState:function(a){return a.replace(/#.*$/,"").replace(/^#+|#+$/,"")},extractAnchor:function(a){return a.replace(/^.*#/,"").replace(/^#+|#+$/,"")},getState:function(){return b.History.state},setState:function(a){var c=b.History;
a=c.extractHash(a);c.state=a;return c.state},getHash:function(){return b.History.extractHash(window.location.hash||location.hash)},setHash:function(a){a=b.History.extractHash(a);if(typeof window.location.hash!=="undefined"){if(window.location.hash!==a)window.location.hash=a}else if(location.hash!==a)location.hash=a;return a},go:function(a){var c=b.History;a=c.extractHash(a);var d=c.getHash(),e=c.getState();if(a!==d)c.setHash(a);else{a!==e&&c.setState(a);c.trigger()}return true},hashchange:function(){var a=
b.History,c=a.getHash();a.go(c);return true},bind:function(a,c){var d=b.History;if(c){if(typeof d.handlers.specific[a]==="undefined")d.handlers.specific[a]=[];d.handlers.specific[a].push(c)}else{c=a;d.handlers.generic.push(c)}return true},trigger:function(a){var c=b.History;if(typeof a==="undefined")a=c.getState();var d,e,f,g;if(typeof c.handlers.specific[a]!=="undefined"){g=c.handlers.specific[a];d=0;for(e=g.length;d<e;++d){f=g[d];f(a)}}g=c.handlers.generic;d=0;for(e=g.length;d<e;++d){f=g[d];f(a)}return true},
construct:function(){var a=b.History;b(document).ready(function(){a.domReady()});return true},configure:function(a){var c=b.History;c.options=b.extend(c.options,a);return true},domReadied:false,domReady:function(){var a=b.History;if(!a.domRedied){a.domRedied=true;a.$window=b(window);a.$window.bind("hashchange",this.hashchange);setTimeout(a.hashchangeLoader,200);return true}},hashchangeLoader:function(){var a=b.History;if(b.browser.msie&&parseInt(b.browser.version)>=8)a.getHash()&&a.$window.trigger("hashchange");
else{var c;if(b.browser.msie){a.$iframe=b('<iframe id="jquery-history-iframe" style="display: none;"></$iframe>').prependTo(document.body)[0];a.$iframe.contentWindow.document.open();a.$iframe.contentWindow.document.close();var d=false;c=function(){var e=a.getHash(),f=a.getState(),g=a.extractHash(a.$iframe.contentWindow.document.location.hash);if(f!==e){if(!d){a.$iframe.contentWindow.document.open();a.$iframe.contentWindow.document.close();a.$iframe.contentWindow.document.location.hash=e}d=false;a.$window.trigger("hashchange")}else if(f!==
g){d=true;a.setHash(g)}}}else c=function(){var e=a.getHash();a.getState()!==e&&a.$window.trigger("hashchange")};setInterval(c,200)}return true}};b.History.construct()}})(jQuery);
(function(b){b(document.body).addClass("js");if(b.Ajaxy)window.console.warn("$.Ajaxy has already been defined...");else{b.Ajaxy={options:{root_url:"",base_url:"",relative_url:"",no_log_class:"ajaxy-no_log",redirect:false,relative_as_base:true,support_text:true,analytics:true,auto_ajaxify:true,auto_ajaxify_documentReady:true,auto_sparkle_documentReady:true,debug:true,Controllers:{}},defaults:{Controller:{selector:null,matches:null,controller:null,response:null,request:null,error:null},Action:{propagate:true,
action:null,state:null,State:null,controller:null,Controller:null,forward:function(){window.console.error("Ajaxy.Action.forward: Forward never defined.",[this,arguments]);window.console.trace()},trigger:function(){window.console.error("Ajaxy.Action.trigger: Trigger never defined.",[this,arguments]);window.console.trace()},stopPropagation:function(){this.propagate=false},preventDefault:function(){this.propagate=false},documentReady:function(a){var c=b.Ajaxy,d=this.State||{};if((a||{}).length)a=b("body");
var e=d.anchor||false;if(e){d.anchor=false;b(".target").removeClass("target");b("#"+e).addClass("target").ScrollTo()}c.options.auto_ajaxify_documentReady&&a.ajaxify();c.options.auto_sparkle_documentReady&&b.Sparkle&&a.sparkle();return true}},State:{log:null,form:false,anchor:null,state:null,controller:null,Request:{url:null,data:{}},Response:{callback:null,data:{}},Error:{callback:null,data:{}},User:{data:{}}}},constructed:false,Controllers:{},States:{},ajaxQueue:[],data:{},redirected:false,extractAnchor:function(a){var c=
b.Ajaxy,d=b.History;a=a.replace(/^\//g,"").stripLeft(c.options.root_url).stripLeft(c.options.base_url);return a=d.extractAnchor(a)},extractState:function(a){var c=b.Ajaxy,d=b.History;a=a.replace(/^\//g,"").stripLeft(c.options.root_url).stripLeft(c.options.base_url);if(a=d.extractState(a))a="/"+a.replace(/^\//g,"");return a},bind:function(a,c){var d=b.Ajaxy;if(typeof c==="undefined"&&typeof a==="object"){b.each(a,d.bind);return true}else if(typeof c==="function")c={response:c};else if(typeof c!=="object"){window.console.error("Ajaxy.bind: Unknown option type",
...
return false}g=b.extend(true,{},e.defaults.Action,{action:c,controller:a,Controller:g,state:d,State:j});g.forward=g.trigger=function(i,n,k){e.options.debug&&window.console.debug("Ajaxy.Action.trigger:",[this,arguments]);i=i||a;n=n||c;k=k||d;e.trigger(i,n,k);return true};h.apply(g,[]);if(g.propagate===false)f=false;f&&a!=="_generic"&&g.forward("_generic");return true},get:function(a){var c=b.Ajaxy;if(typeof c.data[a]!=="undefined")return c.data[a]},set:function(a,c){var d=b.Ajaxy;if(typeof c==="undefined")typeof a===
"object"&&d.data.extend(true,a);else d.data[a]=c},refresh:function(){return b.Ajaxy.go(b.History.getHash())},go:function(a){var c=b.Ajaxy,d=b.History;c.options.debug&&window.console.debug("Ajaxy.go:",[this,arguments]);if(typeof a==="string")a={state:a};var e=c.getState(false,true);b.extend(true,e,a);if(e.state)if(e.form){if(e.log===null||e.log===undefined)e.log=false}else{if(e.log===null||e.log===undefined)e.log=true}else{e.state=c.extractState(e.url);delete e.url;if(e.log===null||e.log===undefined)e.log=
false}e.log=e.log?true:false;if(e.state)e.state=c.extractState(e.state);else{window.console.error("Ajaxy.go: No state",[this,arguments]);return false}e.state===d.getHash()&&c.options.debug&&c.options.debug&&window.console.debug("Ajaxy.go: Trigger but no change.",e.state);c.storeState(e);e.log?d.go(e.state):c.stateChange(e.state);return true},getControllerAction:function(a,c,d){var e=undefined,f=b.Ajaxy.getController(a,false);if(typeof f==="undefined"){if(d!==false){window.console.error("Ajaxy.getControllerAction: Controller does not exist",
...
false){window.console.error("Ajaxy.getState: State does not exist",[this,arguments]);window.console.trace()}return e},track:function(a){var c=b.Ajaxy;if(typeof pageTracker!=="undefined"){a=c.options.base_url+(a.replace(/^\/+/,"")||"?");pageTracker._trackPageview(a)}return true},matches:function(a,c){var d=b.Ajaxy,e=false;switch(typeof a){case "function":case "object":if(a.test){e=a.test(c);break}case "array":b.each(a,function(f,g){if(e=d.matches(g,c))return false});break;case "number":case "string":e=
String(a)===c;break}return e},match:function(a){var c=b.Ajaxy,d=false;b.each(c.Controllers,function(e,f){if(c.matches(f.matches||false,a)){d=e;return false}});return d},request:function(a){var c=b.Ajaxy,d=b.History;c.options.debug&&window.console.debug("Ajaxy.request:",[this,arguments]);var e=false;a=c.extractState(a);if(c.redirected!==false)c.redirected=false;else{c.ajaxQueue.push(a);if(c.ajaxQueue.length!==1)return false;c.options.analytics&&c.track(a);var f=c.getState(a,true),g=f.controller||c.match(a)||
undefined;f.state=a;f.controller=g;f.Request.url=f.Request.url||c.options.root_url+c.options.base_url+(a.replace(/^\/+/,"")||"?");c.storeState(f);c.trigger(g,"request");g={data:f.Request.data,url:f.Request.url,type:"post",success:function(m){c.options.debug&&window.console.debug("Ajaxy.request.success:",[this,arguments]);m=b.extend(true,{},c.defaults.State.Response.data,m);m.Ajaxy=m.Ajaxy||{};if(m.Ajaxy.redirected){var l=c.extractState(m.Ajaxy.redirected.to);c.redirected={status:true,from:a,to:l};
d.go(l)}c.ajaxQueue.shift();if((l=c.ajaxQueue.pop())&&l!==a){c.ajaxQueue=[];c.stateChange(l);return false}f.Response.data=m;f.Error.data={};l=m.controller||f.controller||null;if(l===null){l="_generic";window.console.warn("Ajaxy.request.success.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[m.controller,f.controller])}if(f.Response.callback)if(f.Response.callback.apply(f,arguments)||l==="callback")return true;return c.trigger(l,"response",f)},error:function(m,
l,r,o){c.options.debug&&window.console.debug("Ajaxy.request.error:",[this,arguments]);o||(o={responseText:m.responseText.trim()||false});c.ajaxQueue.shift();var p=c.ajaxQueue.pop();if(p&&p!==a){c.ajaxQueue=[];c.stateChange(p);return false}f.Request.XMLHttpRequest=m;f.Response.data=o;f.Error.data={};p=o.controller||f.controller||null;if(p===null){p="_generic";window.console.warn("Ajaxy.request.error.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[o.controller,
f.controller])}if(f.Error.callback)if(f.Error.callback.apply(f,arguments)||p==="callback")return true;return c.trigger(p,"error",f)},complete:function(m){c.options.debug&&window.console.debug("Ajaxy.request.complete:",[this,arguments]);f.Request.XMLHttpRequest=m}};if(f.form){var h=b(f.form);if(h.attr("enctype")==="multipart/form-data"){e="ajaxy_form_iframe_"+Math.floor(Math.random()*99999);var j=b('<iframe style="display:none" src="about:blank" id="'+e+'" name="'+e+'" >').appendTo("body").hide(),
i=b('<input type="hidden" name="ajax" value="true"/>').appendTo(h),n=b('<input type="hidden" name="Ajaxy[form]" value="true"/>').appendTo(h);j.bind("load",function(){if((this.document||this.currentDocument||this.contentWindow.document).location.href==="about:blank")return true;var m=j.contents().find(".response").val(),l=false;try{l=JSON.parse(m)}catch(r){window.console.error("Ajaxy.request.form: Invalid Response.",[this,arguments],[m])}l?request.success(l):request.error(l);h.removeAttr("target");
j.remove();i.remove();n.remove()});h.attr("target",e);h.submit();var k=h.values();g.data=b.extend(true,g.data,k||{});e=true}else{k=h.values();g.data=b.extend(true,g.data,k||{})}}k=true;f.Request=g;e||(k=c.ajax(g));return k}},ajax:function(a){var c=b.Ajaxy;c.options.debug&&window.console.debug("Ajaxy.ajax:",[this,arguments]);var d={};d.success=a.success||function(){c.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.success:",[this,arguments]);b(".error").empty()};d.error=a.error||function(f,
g,h){c.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.error:",[this,arguments]);b(".error").html(h)};d.complete=a.complete||function(){c.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.complete:",[this,arguments])};delete a.success;delete a.error;delete a.complete;var e={type:"post",dataType:c.options.support_text?"text":"json"};b.extend(true,e,a);e.success=function(f,g){c.options.debug&&window.console.debug("Ajaxy.ajax.request.success:",[this,arguments]);var h={};if(typeof f!==
"object"&&c.options.support_text&&f)try{h=JSON.parse(f)}catch(j){var i=b(f.replace(/<(html|head|body|title)>/g,'<div id="ajaxy-$1">').replace(/<\/(html|head|body|title)/g,"</div>"));h=i.find("#ajaxy-body");var n=i.find("#ajaxy-title");i=i.find("#ajaxy-controller");n=n.length?n.text():null;h=h.length?h.html():f;i=i.length?i.text():null;h={title:n,content:h,controller:i,html:f}}else h=f;c.options.debug&&window.console.debug("Ajaxy.ajax.success:",[this,arguments]);if(typeof h.controller==="undefined"&&
(typeof h.success!=="undefined"&&!h.success||typeof h.error!=="undefined"&&h.error))return d.error.apply(this,[null,g,h.error||true,h]);return d.success.apply(this,[h,g])};e.error=function(f,g,h){c.options.debug&&window.console.debug("Ajaxy.ajax.request.error:",[this,arguments]);var j=f.responseText||false;if(j)j=j.trim();j||(j=false);var i={error:h||true,responseText:j};if(j)try{i=JSON.parse(j)}catch(n){}finally{return this.success.apply(this,[i,g])}return d.error.apply(this,[f,g,h,i])};return b.ajax(e)},
stateChange:function(a){b.Ajaxy.request(a)},configure:function(a){var c=b.Ajaxy;a=a||{};var d=a.Controllers||a.controllers||a;c.options=b.extend(true,c.options,a.options||a||{});c.bind(d);c.options.root_url=(c.options.root_url||document.location.protocol.toString()+"//"+document.location.hostname.toString()).replace(/\/$/,"")+"/";c.options.base_url=c.options.base_url||"";c.options.relative_url=c.extractState(c.options.relative_url||document.location.pathname.toString().replace(/^\//,""));if(c.options.relative_as_base)if(c.options.base_url.length===
0){c.options.base_url=c.options.relative_url;c.options.relative_url=""}if(c.options.base_url)c.options.base_url=c.options.base_url.replace(/^\/|\/$/g,"")+"/";c.options.debug&&window.console.debug("Ajaxy.configure:",[this,arguments]);if(c.options.redirect&&c.options.relative_url&&c.options.relative_url!==null)document.location=c.options.root_url+c.options.base_url+"#"+c.options.relative_url;return true},construct:function(){var a=b.Ajaxy,c=b.History;if(!a.constructed){a.constructed=true;c.bind(function(d){return a.stateChange(d)});
...
a).once("click",c.ajaxify_helpers.a)});return true},ajaxify_helpers:{a:function(a){var c=b.Ajaxy,d=b(this),e=d.attr("href").replace(/^\/?\.\//,"/"),f=c.extractAnchor(e);e=c.extractState(e);if("/"+f===e)f="";var g=!d.hasClass(c.options.no_log_class);d=d.data("ajaxy-controller")||null;c.go({state:e,controller:d,log:g,anchor:f});a.stopPropagation();a.preventDefault();return false},form:function(a){var c=b.Ajaxy,d=b(this),e=d.attr("disabled");if(e=e||e==="false")return false;if(d.attr("target"))return true;
d=c.extractState(d.attr("action"));c.go({state:d,form:this});a.stopPropagation();a.preventDefault();return false}}};b.Ajaxy.construct()}})(jQuery);
