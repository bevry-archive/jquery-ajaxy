...
a[1];a[1]="$.intercept.skipValue"}}else{c=a[0];a[0]="$.intercept.skipValue"}var f={};b.each(c,function(g){f[g]=true});d?b.each(a,function(g,h){if(h==="$.intercept.skipValue")return true;b.each(h,function(i,j){if(typeof f[i]==="undefined")return true;if(typeof j==="object"&&!(j.test||0))b.extend(c[i],j||{});else c[i]=j})}):b.each(a,function(g,h){b.each(h,function(i,j){if(typeof f[i]==="undefined")return true;if(typeof j==="object"&&!(j.test||0))b.intercept(true,c[i],j);else c[i]=j})});return c}})(jQuery);
...
[this,arguments]);a="_generic"}var g=e.getController(a),h=e.getControllerAction(a,c,false),i=e.getState(d,true);d=i.state||undefined;if(typeof g==="undefined"){window.console.error("Ajaxy.trigger: Controller does not exist",[this,arguments]);window.console.trace();a!=="_generic"&&e.trigger("_generic","error",i);return false}if(typeof h==="undefined"){window.console.error("Ajaxy.trigger: No Controller Action",[this,arguments]);window.console.trace();a!=="_generic"&&e.trigger("_generic","error",i);
return false}g=b.extend({},e.defaults.Action,{action:c,controller:a,Controller:g,state:d,State:i});g.forward=g.trigger=function(j,o,m){e.options.debug&&window.console.debug("Ajaxy.Action.trigger:",[this,arguments]);j=j||a;o=o||c;m=m||d;e.trigger(j,o,m);return true};h.apply(g,[]);if(g.propagate===false)f=false;f&&a!=="_generic"&&g.forward("_generic");return true},get:function(a){var c=b.Ajaxy;if(typeof c.data[a]!=="undefined")return c.data[a]},set:function(a,c){var d=b.Ajaxy;if(typeof c==="undefined")typeof a===
"object"&&d.data.extend(true,a);else d.data[a]=c},refresh:function(){return b.Ajaxy.go(b.History.getHash())},go:function(a){var c=b.Ajaxy,d=b.History;c.options.debug&&window.console.debug("Ajaxy.go:",[this,arguments]);if(typeof a==="string")a={state:a};var e=b.extend(true,{},c.defaults.State,a);if(!e.state&&e.url){e.state=c.format(e.url);if(e.log===null||e.log===undefined)e.log=false}else if(e.form){if(e.log===null||e.log===undefined)e.log=false}else if(e.log===null||e.log===undefined)e.log=true;
e.log=e.log?true:false;if(e.state)e.state=c.format(e.state);else{window.console.error("Ajaxy.go: No state",[this,arguments]);return false}e.state===d.getHash()&&c.options.debug&&c.options.debug&&window.console.debug("Ajaxy.go: Trigger but no change.",e.state);c.storeState(e);e.log?d.go(e.state):c.stateChange(e.state);return true},getControllerAction:function(a,c,d){var e=undefined,f=b.Ajaxy.getController(a,false);if(typeof f==="undefined"){if(d!==false){window.console.error("Ajaxy.getControllerAction: Controller does not exist",
...
window.console.trace();d=false}return d},getController:function(a,c){var d=b.Ajaxy,e=undefined,f=typeof(a||undefined);if((f==="number"||f==="string")&&typeof d.Controllers[a]!=="undefined")e=d.Controllers[a];else if(f==="object"&&typeof a.controller==="string")e=d.getController(a.controller,c);else if(c)e=b.extend({},d.defaults.Controller);else if(c!==false){window.console.error("Ajaxy.getController: Controller does not exist",[this,arguments]);window.console.trace()}return e},storeState:function(a){var c=
b.Ajaxy,d=true;if(typeof(a||undefined)==="object"&&typeof a.state==="string")d=c.States[a.state]=a;else{window.console.error("Ajaxy.getState: Unkown State Format",[this,arguments]);window.console.trace();d=false}return d},getState:function(a,c){var d=b.Ajaxy,e=undefined,f=typeof(a||undefined);if((f==="number"||f==="string")&&typeof d.States[a]!=="undefined")e=d.States[a];else if(f==="object"&&typeof a.state==="string")e=d.getState(a.state,c);else if(c)e=b.extend({},d.defaults.State);else if(c!==false){window.console.error("Ajaxy.getState: State does not exist",
[this,arguments]);window.console.trace()}return e},track:function(a){var c=b.Ajaxy;typeof pageTracker!=="undefined"&&pageTracker._trackPageview(c.options.base_url+"/"+a);return true},matches:function(a,c){var d=b.Ajaxy,e=false;switch(typeof a){case "function":case "object":if(a.test){e=a.test(c);break}case "array":b.each(a,function(f,g){if(e=d.matches(g,c))return false});break;case "number":case "string":e=String(a)===c;break}return e},match:function(a){var c=b.Ajaxy,d=false;b.each(c.Controllers,
function(e,f){if(c.matches(f.matches||false,a)){d=e;return false}});return d},request:function(a){var c=b.Ajaxy,d=b.History;c.options.debug&&window.console.debug("Ajaxy.request:",[this,arguments]);var e=false;a=c.format(a);if(c.redirected!==false)c.redirected=false;else{c.ajaxQueue.push(a);if(c.ajaxQueue.length!==1)return false;c.options.analytics&&c.track(a);var f=c.getState(a,true),g=f.controller||c.match(a)||undefined;f.state=a;f.controller=g;f.Request.url=f.Request.url||c.options.root_url+c.options.base_url+
(a.replace(/^\//,"")||"?");c.storeState(f);c.trigger(g,"request");g={data:f.Request.data,url:f.Request.url,type:"post",success:function(k){c.options.debug&&window.console.debug("Ajaxy.request.success:",[this,arguments]);k=b.extend(true,{},c.defaults.State.Response.data,k);k.Ajaxy=k.Ajaxy||{};if(k.Ajaxy.redirected){var l=c.format(k.Ajaxy.redirected.to);c.redirected={status:true,from:a,to:l};d.go(l)}c.ajaxQueue.shift();if((l=c.ajaxQueue.pop())&&l!==a){c.ajaxQueue=[];c.stateChange(l);return false}f.Response.data=
k;f.Error.data={};l=k.controller||f.controller||null;if(l===null){l="_generic";window.console.warn("Ajaxy.request.success.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[k.controller,f.controller])}if(f.Response.callback)if(f.Response.callback.apply(f,arguments)||l==="callback")return true;return c.trigger(l,"response",f)},error:function(k,l,q,p){c.options.debug&&window.console.debug("Ajaxy.request.error:",[this,arguments]);p||(p={responseText:k.responseText.trim()||
false});c.ajaxQueue.shift();var n=c.ajaxQueue.pop();if(n&&n!==a){c.ajaxQueue=[];c.stateChange(n);return false}f.Request.XMLHttpRequest=k;f.Response.data=p;f.Error.data={};n=p.controller||f.controller||null;if(n===null){n="_generic";window.console.warn("Ajaxy.request.error.controller: The controller was unable to be determined, defaulted to _generic.",[this,arguments],[p.controller,f.controller])}if(f.Error.callback)if(f.Error.callback.apply(f,arguments)||n==="callback")return true;return c.trigger(n,
"error",f)},complete:function(k){c.options.debug&&window.console.debug("Ajaxy.request.complete:",[this,arguments]);f.Request.XMLHttpRequest=k}};if(f.form){var h=b(f.form);if(h.attr("enctype")==="multipart/form-data"){e="ajaxy_form_iframe_"+Math.floor(Math.random()*99999);var i=b('<iframe style="display:none" src="about:blank" id="'+e+'" name="'+e+'" >').appendTo("body").hide(),j=b('<input type="hidden" name="ajax" value="true"/>').appendTo(h),o=b('<input type="hidden" name="Ajaxy[form]" value="true"/>').appendTo(h);
i.bind("load",function(){if((this.document||this.currentDocument||this.contentWindow.document).location.href==="about:blank")return true;var k=i.contents().find(".response").val(),l=false;try{l=JSON.parse(k)}catch(q){window.console.error("Ajaxy.request.form: Invalid Response.",[this,arguments],[k])}l?request.success(l):request.error(l);h.removeAttr("target");i.remove();j.remove();o.remove()});h.attr("target",e);h.submit();var m=h.values();g.data=b.extend(g.data,m||{});e=true}else{m=h.values();g.data=
b.extend(g.data,m||{})}}m=true;f.Request=g;e||(m=c.ajax(g));return m}},ajax:function(a){var c=b.Ajaxy;c.options.debug&&window.console.debug("Ajaxy.ajax:",[this,arguments]);var d={};d.success=a.success||function(){c.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.success:",[this,arguments]);b(".error").empty()};d.error=a.error||function(f,g,h){c.options.debug&&window.console.debug("Ajaxy.ajax.callbacks.error:",[this,arguments]);b(".error").html(h)};d.complete=a.complete||function(){c.options.debug&&
window.console.debug("Ajaxy.ajax.callbacks.complete:",[this,arguments])};delete a.success;delete a.error;delete a.complete;var e={type:"post",dataType:c.options.support_text?"text":"json"};b.extend(e,a);e.success=function(f,g){c.options.debug&&window.console.debug("Ajaxy.ajax.request.success:",[this,arguments]);var h={};if(typeof f!=="object"&&c.options.support_text&&f)try{h=JSON.parse(f)}catch(i){h={content:f}}finally{}else h=f;c.options.debug&&window.console.debug("Ajaxy.ajax.success:",[this,arguments]);
if(typeof h.controller==="undefined"&&(typeof h.success!=="undefined"&&!h.success||typeof h.error!=="undefined"&&h.error))return d.error.apply(this,[null,g,h.error||true,h]);return d.success.apply(this,[h,g])};e.error=function(f,g,h){c.options.debug&&window.console.debug("Ajaxy.ajax.request.error:",[this,arguments]);var i=f.responseText||false;if(i)i=i.trim();i||(i=false);var j={error:h||true,responseText:i};if(i)try{j=JSON.parse(i)}catch(o){}finally{return this.success.apply(this,[j,g])}return d.error.apply(this,
[f,g,h,j])};return b.ajax(e)},stateChange:function(a){b.Ajaxy.request(a)},configure:function(a){var c=b.Ajaxy;a=a||{};var d=a.Controllers||a.controllers||a;c.options=b.extend(c.options,a.options||a||{});c.bind(d);c.options.root_url=(c.options.root_url||document.location.protocol.toString()+"//"+document.location.hostname.toString()).replace(/\/$/,"")+"/";c.options.base_url=c.options.base_url||"";c.options.relative_url=c.format(c.options.relative_url||document.location.pathname.toString().replace(/^\//,
""));if(c.options.relative_as_base)if(c.options.base_url.length===0){c.options.base_url=c.options.relative_url;c.options.relative_url=""}if(c.options.base_url)c.options.base_url=c.options.base_url.replace(/^\/|\/$/g,"")+"/";c.options.debug&&window.console.debug("Ajaxy.configure:",[this,arguments]);if(c.options.redirect&&c.options.relative_url&&c.options.relative_url!==null)document.location=c.options.root_url+c.options.base_url+"#"+c.options.relative_url;return true},construct:function(){var a=b.Ajaxy,
c=b.History;if(!a.constructed){a.constructed=true;c.bind(function(d){return a.stateChange(d)});b.fn.ajaxify=a.ajaxify;b.fn.ajaxy=a.ajaxify;b(function(){a.domReady();c.domReady()});return true}},domReady:function(){return true},ajaxify:function(){var a=b.Ajaxy,c=b(this);b.each(a.Controllers,function(d){a.ajaxifyController(d)});c.findAndSelf("a.ajaxy").once("click",a.ajaxify_helpers.a);c.findAndSelf("form.ajaxy").once("submit",a.ajaxify_helpers.form);return c},ajaxifyController:function(a){var c=b.Ajaxy,
d=c.getController(a);d&&typeof d.selector!=="undefined"&&b(function(){b(d.selector).data("ajaxy-controller",a).once("click",c.ajaxify_helpers.a)});return true},ajaxify_helpers:{a:function(a){var c=b.Ajaxy,d=b(this),e=c.format(d.attr("href").replace(/^\/?\.\//,"/")),f=!d.hasClass(c.options.no_log_class);d=d.data("ajaxy-controller")||null;c.go({state:e,controller:d,log:f});a.stopPropagation();a.preventDefault();return false},form:function(a){var c=b.Ajaxy,d=b(this),e=d.attr("disabled");if(e=e||e===
"false")return false;if(d.attr("target"))return true;d=c.format(d.attr("action"));c.go({state:d,form:this});a.stopPropagation();a.preventDefault();return false}}};b.Ajaxy.construct()}})(jQuery);
